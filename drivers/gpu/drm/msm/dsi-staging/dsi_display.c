...
static irqreturn_t dsi_display_panel_err_flag_irq_handler(int irq, void *data)
{
	 ...
	 */
	if ((!display) || (!display->panel->is_err_flag_irq_enabled) ||
...
static void dsi_display_register_err_flag_irq(struct dsi_display *display)
{
	...
	rc = devm_request_irq(dev, err_flag_irq,
			      dsi_display_panel_err_flag_irq_handler,
	...
	if (strcmp(panel->name, "samsung dsc cmd mode oneplus dsi panel") == 0) {
		...
		if (!count) {
			...
			rc = dsi_panel_tx_cmd_set_op(
				panel, DSI_CMD_READ_SAMSUNG_PANEL_REGISTER_ON);
		}
		...
		if (rc <= 0)
			pr_err("rx cmd transfer failed rc=%d\n", rc);
		rc = dsi_panel_tx_cmd_set_op(
			panel, DSI_CMD_READ_SAMSUNG_PANEL_REGISTER_OFF);
		...
		if (((temp_buffer_6[0] == 132) && (temp_buffer_6[1] == 0)) ||
		    (temp_buffer_1[0] != 159))
			rc = -1;
		else
			rc = 1;
		...
		if ((register_0a[0] != 0x9c) || (register_b6[0] != 0x0a)) {
			...
			envp[0] = kasprintf(GFP_KERNEL, "ESD_BLACK_PANEL=%d",
					    esd_black_count);
			envp[1] = kasprintf(GFP_KERNEL, "ESD_GREENISH_PANEL=%d",
					    esd_greenish_count);
			envp[2] = NULL;
			kobject_uevent_env(&display->pdev->dev.kobj,
					   KOBJ_CHANGE, envp);
					...
					bool te_check_override)
{
	...
	if (dsi_display->panel->err_flag_status == true) {
		...
		envp[1] = kasprintf(GFP_KERNEL, "ESD_GREENISH_PANEL=%d",
				    esd_greenish_count);
		envp[2] = NULL;
		kobject_uevent_env(&dsi_display->pdev->dev.kobj, KOBJ_CHANGE,
				   envp);
			       ...
			       size_t count, loff_t *ppos)
{
	...
	ret = simple_read_from_buffer(user_buf, count, ppos, fresh_rate,
				      strlen(fresh_rate));
	return ret;
}
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	if (fresh_rate_input_dev_init == false) {
		//create fresh_rate
		proc_entry_tmp =
			proc_create("fresh_rate", 0664, proc_entry_display,
				    &fresh_rate_fops);
		...
		if (dsi_display_mode_switch_dfps(cur_mode, adj_mode)) {
			...
			if (mode_fps != adj_mode->timing.refresh_rate) {
				...
				if (fresh_rate_report_enable) {
					input_event(fresh_rate_input_dev,
						    EV_MSC, MSC_RAW, mode_fps);
...
int dsi_display_set_acl_mode(struct drm_connector *connector, int level)
{
	...
	rc = dsi_display_clk_ctrl(dsi_display->dsi_clk_handle, DSI_CORE_CLK,
				  DSI_CLK_ON);
	if (rc) {
		pr_err("[%s] failed to enable DSI core clocks, rc=%d\n",
		       dsi_display->name, rc);
			       ...
			       struct dsi_panel *panel)
{
	...
	do {
		...
		for (i = 0; i < 452; i++) {
			rc = dsi_panel_tx_cmd_set(
				panel, DSI_CMD_SET_GAMMA_FLASH_PRE_READ_1);
			if (rc) {
			...
			rc = dsi_panel_tx_cmd_set(
				panel, DSI_CMD_SET_GAMMA_FLASH_PRE_READ_2);
			if (rc) {
			...
			flags = 0;
			cmds = mode->priv_info
				       ->cmd_sets[DSI_CMD_SET_GAMMA_FLASH_READ_FB]
				       .cmds;
			if (cmds->last_command) {
				cmds->msg.flags |= MIPI_DSI_MSG_LASTCOMMAND;
				flags |= DSI_CTRL_CMD_LAST_COMMAND;
				   ...
				   struct dsi_panel *panel, char *buf, int len)
{
	...
	if ((strcmp(dsi_display->panel->name,
		...
		flags = 0;
		cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_PRODUCTION_INFO]
			       .cmds;
		if (cmds->last_command) {
			cmds->msg.flags |= MIPI_DSI_MSG_LASTCOMMAND;
			flags |= DSI_CTRL_CMD_LAST_COMMAND;
...
int dsi_display_get_serial_number_AT(struct drm_connector *connector)
{
	...
	if (count) {
		...
		serial_number  =      ((uint64_t)panel_year << 56)
							...
							+ ((uint64_t)0);
		*/
		serial_number = (uint64_t)panel_year * 10000000000 +
				(uint64_t)panel_mon * 100000000 +
				(uint64_t)panel_day * 1000000 +
				(uint64_t)panel_hour * 10000 +
				(uint64_t)panel_min * 100 + (uint64_t)panel_sec;
...
int dsi_display_set_aod_mode(struct drm_connector *connector, int level)
{
	...
	if ((dsi_display->panel->aod_mode != 5) &&
	    (dsi_display->panel->aod_mode != 4)) {
...
static int dsi_display_get_mipi_dsi_msg(const struct mipi_dsi_msg *msg,
					char *buf)
{
	int len = 0;
	size_t i;
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%02X %02X ",
			msg->tx_len >> 8, msg->tx_len & 0x00FF);
...
int dsi_display_get_dsi_on_command(struct drm_connector *connector, char *buf)
{
	...
	for (i = 0; i < cmd->count; i++) {
		count += dsi_display_get_mipi_dsi_msg(&cmd->cmds[i].msg,
						      &buf[count]);
	}
					 ...
					 const char *buf, size_t count)
{
	...
	rc = dsi_panel_update_cmd_sets_sub(set, DSI_CMD_SET_PANEL_COMMAND, data,
					   length);
	if (rc)
		pr_err("Failed to update_cmd_sets_sub, rc=%d\n", rc);
	kfree(data);
				      ...
				      char *buf)
{
	...
	cmd = &dsi_display->panel->cur_mode->priv_info
		       ->cmd_sets[DSI_CMD_SET_PANEL_COMMAND];
...
int dsi_display_get_dsi_seed_command(struct drm_connector *connector, char *buf)
{
	...
	cmd = &dsi_display->panel->cur_mode->priv_info
		       ->cmd_sets[DSI_CMD_SET_SEED_COMMAND];
